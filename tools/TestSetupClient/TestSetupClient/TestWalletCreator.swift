...
class TestWalletCreator: ObservableObject {
    private static let minFee: UInt64 = 400_000_000
    private static let minMOBUSDFee: UInt64 = 2650

    private static var testAccountConfigs: [TestAccountFactory.TestAccountConfig] {
        [
    ...
    private static func makeConfig() -> MobileCoinClient.Config? {
    ...
    final func createAccounts(
        srcAcctEntropyString: String,
        testAccountSeed: String
    ) async -> Result<Void, TestWalletCreationError> {
        do {
            guard let fogAuthoritySpki = Data(
                base64Encoded: NetworkPresets.fogAuthoritySpkiB64Encoded) else {
                return .failure(.error("Unable to get fogAuthoritySpki"))
            }

            guard let srcAcctEntropyData = Data(
                base64Encoded: srcAcctEntropyString,
                options: []) else {
            ...
            let testAccounts = try await accountFactory.makeAccounts(
                ...
                testAccountConfigs: Self.testAccountConfigs,
